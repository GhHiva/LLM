...
**Notes:**
  - list('emma') = ['e','m','m','a']
  - act like indicator to show where the start and end of word is.
  - b.get(bigram,0): is used to show the values of the keys,. If there is no value it returns 0.
  - sorted(b.items): sorts based on the keys. for example (, 'a') goes first than (, 'e').
     - sorted(b.items(), reverse = True): for example (, 'e') goes before (, 'a').
